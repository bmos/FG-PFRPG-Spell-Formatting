...
local function getReferenceSpell(string_spell_name)
	...
	for s, v in pairs(tFormats) do
		if tTrims[v] then
			string_spell_name = string.format("%s, %s", string_spell_name, s)
		end
	end

	return DB.findNode(string.format("spelldesc.%s@*", string_spell_name))
		or DB.findNode(string.format("reference.spells.%s@*", string_spell_name))
end
					...
					if nodeLinkedSpells.getChildCount() > 0 then
						...
						for _, v in ipairs(DB.getChildList(nodeLinkedSpells)) do
							...
							sValue = string.format(
								"%s<link class='%s' recordname='%s'>%s</link>",
								sValue,
								sLinkClass,
								sLinkRecord,
								sLinkName
...
local function updateSpellDescString(nodeSpell)
	...
	if nodeDesc then
		...
		if sDescType == "formattedtext" then
			...
			if nodeLinkedSpells then
				local nIndex = 1
				local nLinkStartB, nLinkStartE, sClass, sRecord =
					string.find(sValue, "<link class='([^']*)' recordname='([^']*)'>", nIndex)
				while nLinkStartB and sClass and sRecord do
					...
					if nLinkEndB then
						...
						nIndex = nLinkEndE + 1
						nLinkStartB, nLinkStartE, sClass, sRecord =
							string.find(sValue, "<link class='([^']*)' recordname='([^']*)'>", nIndex)
					else
						nLinkStartB = nil
					end
